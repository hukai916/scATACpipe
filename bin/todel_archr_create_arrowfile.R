...
correct_barcode <- function(barcode_file, whitelist_file, reads_per_chunk, path_output_fq) {
	...
	for (i in seq(wb_wid)) {
		...
		keys1mismatch 	<- vector(mode = "list", length = 4 * wb_num)
		values1mismatch <- vector(mode = "list", length = 4 * wb_num)
		count <- 0
	...
	summary_info <- paste0("Summary: total valid: ", total_valid, "; total corrected (1 mismatch): ", total_1mismatch, "; total discarded: ", total_discard, ".")
