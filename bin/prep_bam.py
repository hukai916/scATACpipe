...
"""
Created on 12/04/20221
@authors: liuh, huk

Usage:
python prep_bam.py -h
"""
...
def prep_bam(contig, inbam, header_len_dict,
           soft_clip_5, soft_clip_3,
           shift_forward = 4, shift_reverse = -5,
           barcode_regex = "[^:]+",
           barcode_tag_to_add = "CB",
           outdir = ".",
           barcode_tag = "N/A"):
                ...
                if left_read_check and right_read_check:
                    ...
                    try:
                        if barcode_tag == "N/A":
                            ...
                            left_read.set_tag(tag = barcode_tag_to_add, value = cell_barcode, value_type = "Z")
                            right_read.set_tag(tag = barcode_tag_to_add, value = cell_barcode, value_type = "Z")
                        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Prepare BAM: extend soft-clips, add barcode tag, shift reads for Tn5, remove incorrect reads (with no cellbarcodes or improperly mapped).')
    ...
    parser.add_argument('--barcode_tag_to_add', type = str, default = 'CB',
                            help = 'The name of tag to assign cell barcode (parsed out from seq name using --barcode_regex) to. (default: "%(default)s")')
    ...
    with Pool(nproc) as p:
        contig_bam_lists = p.map_async(functools.partial(prep_bam, inbam = args.inbam,
                                                    ...
                                                    barcode_tag_to_add = args.barcode_tag_to_add,
    ...
    for bam in contig_bam_files:
        ...
        # pysam.sort("-o", outname, "-m", "4G", "-@", nproc, bam)
        pysam.sort("-o", outname, "-@", nproc, bam)
        pysam.index(outname)
        out_bams.append(outname)

    merge_param = ["-f", "-@", nproc, args.outbam] +  out_bams
    ...
    summary = ("Summary (prep_bam.py): soft clips extended: " + str(soft_clip_num) + ", with no barcodes in seq name or in tag: " + no_corrected_barcode_num + ", improperly mapped reads: " + str(not_properly_mapped_num) + ".")
    print(summary)
    with open("summary_prep_bam_" + os.path.basename(args.inbam) + ".txt", "w") as f:
        f.write(summary)
