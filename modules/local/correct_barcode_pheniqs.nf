...
process CORRECT_BARCODE_PHENIQS {
    ...
    # cat ${sample_name}.tag.tsv | awk 'BEGIN { OFS = "\\t"} { print \$1,"CB",\$2 }' > ${sample_name}.tagfile_sinto.tsv

    # Step5, print stats:
    valid_read_num=\$(cat ${sample_name}.tag.tsv | awk '{ if (\$1 == \$2) print \$0 }' | wc -l)
    discard_read_num=\$(cat ${sample_name}.tag.tsv | awk '{ if (\$2 == "undetermined") print \$0 }' | wc -l)
    rescued_read_num=\$(cat ${sample_name}.tag.tsv | grep -v "undetermined" | awk '{ if (\$1 != \$2) print \$0 }' | wc -l)
