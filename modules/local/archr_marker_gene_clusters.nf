...
process ARCHR_MARKER_GENE_CLUSTERS {
  // Find marker genes for "Clusters" only.
    ...
    ## in case gene symbols are appended with gene_id, like: geneSymbl_geneID
    ...
    if (length(markerGenes2labeled) == 0) {
      ...
      tem <- make.names(markersGS@elementMetadata@listData\$name, unique = TRUE) # otherwise markerHeatmap will not plot label
      markersGS@elementMetadata@listData\$name <- tem
