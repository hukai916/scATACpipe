...
include { TAG_BAM          } from '../modules/local/tag_bam'  addParams( options: modules['tag_bam'] )
include { DEDUP_BAM        } from '../modules/local/dedup_bam' addParams( options: modules['dedup_bam'] )
include { DEDUP_BAM2       } from '../modules/local/dedup_bam2' addParams( options: modules['dedup_bam2'] )
...
workflow PREPROCESS_DEFAULT {
  ...
  main:
    ...
    // DEDUP_BAM2 module:
    try {
      res_files = res_files.mix(DEDUP_BAM2.out.dedup_summary.collect().ifEmpty([]))
    } catch (Exception ex) {}
    // CUTADAPT module:
    try {
